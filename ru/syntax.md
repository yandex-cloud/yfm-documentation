...
Если вам нужно, чтобы автоматически сгенерированные якоря совпадали с GitHub, например, кириллица не транслитирировалась, то можете включить в [настройках YFM](./settings.md) опцию `supportGithubAnchors: true`. Тогда  будут одновременно генерироваться два вида якорей: и в YFM, и в GitHub стиле.
...
Если вы привыкли к непереносу строк по переводу каретки в разметки, то можете выключить в [настройках YFM](./settings.md) опцию `breaks: false`. Тогда для переноса строк нужно будет ставить в конце строки два или более пробелов.
...
Для _курсивного_ начертание текста, заключите его в знаки подчеркивания:
...
Чтобы ~~зачеркнуть текст~~, заключите его в `~~`:
...
TODO: нотис о максимальной длине инлайнового блока кода.
...
Email адреса и URL можно быстро превратить в ссылку обернув их в `<>`.
...
Вы можете сделать свои ссылки проще и читабельнеее используя специальные reference-style ссылки. Такая ссылка состоит из двух частей: короткого описания в тексте и URL описанном в другом месте документа.
...
В YFM вы можете объявлять и использовать переменные. При сборке переменные будут подставлены в текст документации или использованы для вычисления условий. Это удобно для сборки документации разных версий сервиса из одних и тех же исходных файлов или, например, можно унести в переменные часто повторяющие константы, такие как, названия продуктов.

Переменные можно передавать при сборке документации или описать в [файлах пресетов](./project/presets.md).

{% note tip "Отключить переменные" %}

Всю функциональность связанную с переменными можно выключить в [настройках YFM](./settings.md) с помощью опции `disableLiquid: true`.
...
TODO: опять забыл почему это называется фильтры.
...
TODO: кажется, в тестовом наборе ошибка, Alice должна быть с маленькой буквы
...
TODO: Latex и диаграммы. Вероятно, им не нужны отдельные страницы, можно описать тут.
...
По-умолчанию, YFM экранирует HTML. Если вы хотите все-равно его использовать, то вы можете выключить экранирование в [настройках YFM](./settings.md) с помощью опцию `allowHTML: true`.
